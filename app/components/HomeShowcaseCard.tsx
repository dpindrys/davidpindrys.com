...
type HomeShowcaseCardProps = {
  ...
  ctaLabel?: string;
  /** Vertical image-over-copy layout for half-width grids (e.g. Work page). */
...
function ShowcaseCardCopy({
  ...
  const titleClass = compact
    ...
    : "w-full font-sans text-[clamp(20px,2.1vw,26px)] font-semibold leading-[1.2] tracking-[-0.02em] text-black";
  const bodyClass = compact
    ? "font-sans text-[14px] font-normal leading-[1.55] text-black/70 md:text-[15px]"
    : "w-full font-sans text-[15px] font-normal leading-[1.6] text-black/70 md:text-[16px]";

  return (
    <>
      <div className={`flex w-full flex-col ${compact ? "gap-3" : "gap-4"}`}>
        <h3 className={titleClass}>{title}</h3>
        {descriptor ? <p className={bodyClass}>{descriptor}</p> : null}
      </div>
  ...
  if (layout === "stacked") {
    return (
      <Link
        ...
        className={`${cardLinkClass} flex h-full min-h-0 flex-col`}
      >
        <div
          className={`flex min-h-[min(44vw,240px)] w-full items-center justify-center bg-white md:min-h-[200px] ${showcaseImagePanelPaddingClass}`}
        >
        ...
        <div className="flex flex-1 flex-col justify-center gap-4 px-6 py-7 md:px-7 md:py-8">
          <ShowcaseCardCopy
            title={title}
